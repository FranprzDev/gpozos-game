    ...
    <main ref={scope} className="game-shell">
      ...
      {saved && (
        <section className="result-stage">
          ...
          <div className="result-bottom"><div className="ideal-plan"><span>Tu configuración</span>{selected.map(id => { const e = edges.find(edge => edge.id === id)!; return <div className="player-reveal" key={`player-${id}`}>{nodes.find(n => n.id === e.from)!.label} <b>→</b> {nodes.find(n => n.id === e.to)!.label}</div>; })}<span className="model-label">Configuración óptima del modelo PL</span><div className="ideal-reveal">Pozo 45 <b>→</b> El Paraíso</div><div className="ideal-reveal">Pozo 138 <b>→</b> San Lorenzo</div><div className="ideal-reveal">Pozo 41 <b>→</b> El Bosque</div></div><div className="result-actions"><strong>{matches} / 3 coincidencias</strong><button className="replay-button" onClick={downloadGif}>↓ Descargar animación (.gif)</button><a className="confirm final-link" href="/ranking">Ver ranking →</a><button className="cancel" onClick={() => location.reload()}>Jugar de nuevo</button></div></div>
        </section>
      )}
    </main>
  );
