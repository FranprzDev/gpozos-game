      ...
      {saved && (
        <section className="result-stage">
          ...
          <svg className="network result-network" viewBox="0 0 220 100" aria-label="Comparación entre tu plan y la solución ideal">
            {edges.map(e => { const a = nodes.find(n => n.id === e.from)!; const b = nodes.find(n => n.id === e.to)!; const chosen = selected.includes(e.id); const match = chosen && e.optimal; return <path key={e.id} className={`pipe ${match ? "player-match" : chosen ? "player-miss" : ""}`} d={edgePath(a, b, e.lane, 4.2, 4.2)} />; })}
