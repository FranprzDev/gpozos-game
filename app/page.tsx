      ...
      {saved && (
        <section className="result-stage">
          ...
          <svg className="network result-network" viewBox="0 0 220 100" aria-label="Comparación entre tu plan y la solución ideal">
            {edges.map(e => { const a = nodes.find(n => n.id === e.from)!; const b = nodes.find(n => n.id === e.to)!; return <path key={e.id} className={`pipe ${selected.includes(e.id) ? "locked" : ""}`} d={edgePath(a, b, e.lane)} />; })}
